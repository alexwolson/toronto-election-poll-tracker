...
The signature forecast module is three stacked views of one set of simulated elections, in a fixed order: the margin between the two poll leaders as three named outcomes (leader ahead by two or more points, within two points either way, challenger ahead by two or more), each a ruled row with a plain-language label, a bar scaled to the largest outcome and its exact share in serif metric type; each candidate's election-day vote range (a dot at the middle estimate on a line spanning the central 80% of outcomes); and whole-percent chances of winning the full race on a ruled list. Beneath them, when the feed carries it, a fourth view shows where the uncertainty comes from: the leader margin at three snapshots of the same simulations (what the polls say today, after the rest of the campaign, on election day), each a ruled row with a range bar on one shared axis with the tie marked, tinted by which candidate is ahead, a dark tick at the middle, and the challenger's chance of being ahead in serif metric type at the right; the range widens row by row while the tick barely moves, and the last row is the published margin. Candidate identity lives in the marker and colour; chances are rounded to whole percentages with "<1%" and ">99%" guarding the tails. Bands and frequency phrases are retired.
