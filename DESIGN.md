...
The signature forecast module leads with one chart of one set of simulated elections: the margin between the two poll leaders as three named outcomes (leader ahead by two or more points, within two points either way, challenger ahead by two or more), each a ruled row with a plain-language label, a bar scaled to the largest outcome and its exact share in serif metric type. Beneath it, two further views of the same simulations sit behind real tabs (serif tab labels on a hairline, the selected one underlined in the strong rule colour; arrow keys move between them; every panel is in the markup and the inactive one is hidden): "What the vote could look like", each candidate's election-day vote range as a dot at the middle estimate on a line spanning the central 80% of outcomes; and "Where the uncertainty comes from", the leader margin at three snapshots (what the polls say today, after the rest of the campaign, on election day), each a ruled row with a range bar on one shared axis with the tie marked, tinted by which candidate is ahead, a dark tick at the middle, and the challenger's chance of being ahead in serif metric type at the right; the range widens row by row while the tick barely moves, and the last row is the published margin. A feed without the uncertainty block shows the vote ranges as a plain ruled section instead of tabs. The full-race win list is retired as redundant with the lede and the margin caption. Candidate identity lives in the marker and colour; chances are rounded to whole percentages with "<1%" and ">99%" guarding the tails. Bands and frequency phrases are retired.
